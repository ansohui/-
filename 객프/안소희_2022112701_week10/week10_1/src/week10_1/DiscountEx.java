...
public class DiscountEx {
	...
	public static void main(String[] args) {
		...
		while(true) {
			...
			System.out.println("discountValue 설정 값을 입력하세요");//할인 값 입력받음 
			double dDiscount = oInDev.nextDouble();
			
			System.out.println("할인 정책을 선택하세요(1. 고정 금액 할인, 2. 퍼센트 할인)");
			int iChoice = oInDev.nextInt();
			if(iChoice == 1) {
				DiscountPolicy fixedPolicy = new FixedDiscountPolicy(dDiscount);//업캐스팅 
		        ...
		        DiscountPolicy percentPolicy = new PercentDiscountPolicy(dDiscount);//업캐스팅  
