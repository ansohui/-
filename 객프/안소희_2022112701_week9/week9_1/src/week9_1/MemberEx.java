	...
	public static void main(String[] args) {
		...
		for( i = 0 ; i < iMemberNum; i++) {
			...
			while (!oInDev.hasNextInt()) {
				System.out.println("ID 숫자를 입력하시오 ");//숫자로 입력하도록 함 
		...
		for(i = 0 ; i < iCrewNum; i++) {
			System.out.print("student ID:");//학번 
			while (!oInDev.hasNextInt()) {
                System.out.println("ID 숫자를 입력하시오 ");//숫자 입력받도록 에러메세지 
		...
		for(i = 0 ; i < iMemberNum; i++) {
			String sResult = Members[i].toString();//toString메소드를 통해 문자열(출력 정보)을 받음 
			System.out.println(sResult);//그 문자열을 출력한다 
			System.out.println("");
		}
		System.out.println("---Input Crews---");//입력받은 집행부원 전체 정보 출력 
		for(i = 0 ; i < iCrewNum; i++) {
			String sResult = Crews[i].toString();//toString메소드를 통해 문자열(출력 정보)을 받음 
			System.out.println(sResult);//그 문자열을 출력한다 
			System.out.println("");
		}
		
		oInDev.close();//사용자의 입력을 받는 oInDev객체 닫음 
